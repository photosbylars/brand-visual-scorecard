...
const QUESTIONS = [
  {
    ...
    labels: ["Our photos don't even come close", "There's a noticeable gap between us and them", "We're roughly in the same range", "We'd hold our own side by side", "We'd stand out as the stronger brand"],
  },
  {
    ...
    labels: ["We look like we just launched yesterday", "Visuals feel a bit scrappy and unpolished", "We look professional but not premium", "We look like a well-established brand", "Our visuals signal category leadership"],
  },
  {
    ...
    labels: ["We only have one or two basic photos", "We're missing key angles and details", "Decent coverage but with clear gaps", "Strong set with only minor gaps", "Every angle and detail is covered"],
  },
  {
    ...
    labels: ["They look like completely different brands", "Recognizable but visually inconsistent", "Mostly aligned with some noticeable gaps", "Cohesive with only minor differences", "Seamless and unmistakably one brand"],
  },
  {
    ...
    labels: ["We don't use any motion content at all", "We've tried it once or twice but nothing consistent", "We use it occasionally without a clear system", "We produce motion content regularly", "Motion is a core part of our content strategy"],
  },
  {
    ...
    labels: ["Our photos actively undercut our pricing", "There's a slight disconnect between visuals and price", "Neutral, our photos neither help nor hurt", "Our visuals reinforce that we're worth it", "Our photography elevates the perceived value"],
  },
  {
    category: "PRODUCT PHOTOGRAPHY",
    ...
    labels: ["We only have white background product shots", "A few lifestyle images but they're inconsistent", "Some lifestyle content but no real system", "We have a solid library of lifestyle images", "We have a full lifestyle content system"],
  },
  {
    category: "BRAND ELEVATION",
    ...
    labels: ["We completely blend in with everyone else", "It would take some digging to see the difference", "There are hints of differentiation but nothing strong", "Our visual identity is noticeably distinct", "We're unmistakable, you'd know us anywhere"],
  },
  {
    category: "MOTION CONTENT",
    ...
    labels: ["We'd be completely outmatched", "We'd definitely feel the pressure", "We'd be a little concerned but not panicked", "We'd be on roughly equal footing", "We're already ahead on motion content"],
  },
  {
    category: "E-COMMERCE OPTIMIZATION",
    ...
    labels: ["We've never updated our original photos", "We only shoot new photos at product launch", "We update when something starts feeling dated", "We refresh annually or with seasonal campaigns", "We have a continuous content refresh cycle"],
  },
  {
    category: "BRAND CONSISTENCY",
    ...
    labels: ["Every shoot looks completely different", "There are noticeable style differences between shoots", "Somewhat consistent but you can tell they're different", "Mostly cohesive with minor variations", "Every image looks like it came from the same system"],
  },
  {
    category: "CONTENT SYSTEM",
    ...
    labels: ["It's purely product shots, nothing more", "Product photos with basic styling and props", "There's some brand personality coming through", "We have a clear visual narrative and point of view", "Our content builds a full brand world people want to be part of"],
  },
];

const CATEGORY_LIST = ["PRODUCT PHOTOGRAPHY", "BRAND CONSISTENCY", "E-COMMERCE OPTIMIZATION", "MOTION CONTENT", "CONTENT SYSTEM", "BRAND ELEVATION"];
...
async function submitToKit({ email, firstName, brandName, score, tier, weakest, catScores, scoreShape, utmParams }) {
  try {
    const keyMap = {
    ...
    const catFields = {};
    catScores.forEach(c => { catFields[keyMap[c.name]] = String(c.score) + "/" + String(c.max); });
    await fetch(`https://api.convertkit.com/v3/forms/${KIT_FORM_ID}/subscribe`, {
      ...
      body: JSON.stringify({
        ...
        fields: {
          brand_name: brandName, scorecard_score: String(score), scorecard_tier: tier,
          weakest_category: weakest, score_shape: scoreShape, ...catFields,
          traffic_source: [utmParams.utm_source, utmParams.utm_medium, utmParams.utm_campaign].filter(Boolean).join(" / ") || "direct",
        },
      }),
    });
    return true;
...
function gtag() { if (typeof window !== "undefined" && window.gtag) window.gtag(...arguments); }
...
export default function BrandVisualScorecard() {
  ...
  useEffect(() => {
    if (screen === "results_preview" || screen === "full_results") {
      let frame = 0;
  ...
  useEffect(() => {
    if (screen !== "intro" && screen !== "questions") {
      try { gtag("event", "scorecard_" + screen, { score: totalScore, tier: tier.label }); } catch {}
    }
  }, [screen]);
  ...
  const handleAnswer = (val) => {
    ...
    if (currentQ < QUESTIONS.length - 1) {
      ...
      setTimeout(() => setScreen("results_preview"), 400);
    }
  };
  ...
  const handleEmailSubmit = async () => {
    ...
    const cs = getCategoryScores(answers);
    const weakestCat = [...cs].sort((a, b) => a.pct - b.pct)[0]?.name || "";
    await submitToKit({ email, firstName, brandName, score, tier: tierLabel, weakest: weakestCat, catScores: cs, scoreShape: analyzeScoreShape(cs, brandName), utmParams });
    try { gtag("event", "scorecard_email_submitted", { score, tier: tierLabel, source: utmParams.utm_source || "direct" }); } catch {}
    setSubmitting(false);
    ...
    return (
      <div style={baseStyle}>
        ...
        <div style={{ paddingTop: 60, paddingBottom: 40 }}>
          ...
          <p style={{ fontSize: 17, color: "#444", maxWidth: 480, marginBottom: 12 }}>Score your brand's visual presence across 6 critical dimensions. Get a personalized breakdown with benchmarks against top-performing DTC brands, plus 3 actionable next steps sent to your inbox.</p>
          ...
          <button onClick={() => { if (brandName.trim()) { setScreen("questions"); try { gtag("event", "scorecard_started", { brand: brandName }); } catch {} } }} style={{
        ...
        <div style={{ paddingTop: 32 }}>
          ...
          <div key={currentQ} style={{ animation: "fadeIn 0.25s ease" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {q.labels.map((label, i) => {
                ...
                return (
                  ...
                  >
                    <span style={{ width: 28, height: 28, borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, fontWeight: 700, flexShrink: 0, background: isSelected ? WHITE : "transparent", color: isSelected ? BLUE_DARK : "#bbb", border: isSelected ? "none" : "1.5px solid #ddd" }}>{val}</span>
                    {label}
                  </button>
                );
              })}
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 20 }}>
              {currentQ > 0 ? (<button onClick={() => setCurrentQ(currentQ - 1)} style={{ padding: "8px 0", background: "none", border: "none", cursor: "pointer", fontSize: 13, color: "#999", fontFamily: "'Pontano Sans', sans-serif" }}>← Previous</button>) : <span />}
              {canGoForward && (<button onClick={() => setCurrentQ(currentQ + 1)} style={{ padding: "8px 0", background: "none", border: "none", cursor: "pointer", fontSize: 13, color: BLUE_DARK, fontFamily: "'Pontano Sans', sans-serif", fontWeight: 600 }}>Next →</button>)}
            </div>
          </div>
        </div>
  ...
  if (screen === "results_preview") {
    const isValid = email.includes("@") && email.includes(".") && firstName.trim();
    const sorted = [...catScores].sort((a, b) => b.pct - a.pct);
    const visible = sorted.slice(0, 3);
    const locked = sorted.slice(3);
    const lockedLabel = locked.map(c => c.name.charAt(0) + c.name.slice(1).toLowerCase()).join(", ");
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
                {locked.map((cat) => (
                  <div key={cat.name}>
                    ...
                    <ScoreBar pct={cat.pct} color={cat.pct >= 40 ? "#E67E22" : "#C0392B"} delay={0} />
                  </div>
                ))}
              </div>
            </div>
            <div style={{ position: "absolute", top: 0, left: 0, right: 0, bottom: 0, display: "flex", alignItems: "center", justifyContent: "center" }}>
              <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 6, background: WHITE, padding: "12px 24px", borderRadius: 2, border: `1.5px solid ${BLUE_DARK}` }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke={BLUE_DARK} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                    <rect x="3" y="11" width="18" height="11" rx="2" ry="2" />
                    <path d="M7 11V7a5 5 0 0110 0v4" />
                  </svg>
                  <span style={{ fontSize: 12, fontWeight: 600, letterSpacing: "0.08em", textTransform: "uppercase", color: BLUE_DARK }}>Unlock to reveal</span>
                </div>
                <span style={{ fontSize: 12, color: "#888" }}>{lockedLabel}</span>
              </div>
            </div>
          </div>

          <div style={{ padding: "36px 24px", background: BLACK, color: WHITE, textAlign: "center", marginBottom: 24 }}>
            <div style={{ ...headingStyle, fontSize: 22, marginBottom: 8 }}>UNLOCK YOUR FULL RESULTS</div>
            <p style={{ fontSize: 14, color: "#ccc", maxWidth: 420, margin: "0 auto 24px", lineHeight: 1.5 }}>
              Reveal your scores for {lockedLabel}, plus get a personalized action plan with concrete next steps for your weakest areas.
            </p>
            <div style={{ maxWidth: 340, margin: "0 auto" }}>
              <div style={{ marginBottom: 12 }}>
                <input type="text" value={firstName} onChange={e => setFirstName(e.target.value)} placeholder="Your first name" style={{ ...inputStyle, background: "rgba(255,255,255,0.08)", color: WHITE, border: "1.5px solid rgba(255,255,255,0.2)" }}
                  onFocus={e => e.target.style.borderColor = BLUE_LIGHT} onBlur={e => e.target.style.borderColor = "rgba(255,255,255,0.2)"} />
              </div>
              <div style={{ marginBottom: 16 }}>
                <input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="you@yourbrand.com" style={{ ...inputStyle, background: "rgba(255,255,255,0.08)", color: WHITE, border: "1.5px solid rgba(255,255,255,0.2)" }}
                  onFocus={e => e.target.style.borderColor = BLUE_LIGHT} onBlur={e => e.target.style.borderColor = "rgba(255,255,255,0.2)"} />
              </div>
              <button onClick={handleEmailSubmit} disabled={!isValid || submitting} style={{
                ...headingStyle, width: "100%", fontSize: 14, letterSpacing: "0.12em", padding: "16px 40px",
                background: isValid && !submitting ? BLUE_DARK : "rgba(255,255,255,0.15)", color: WHITE, border: "none",
              ...
              >{submitting ? "UNLOCKING..." : "UNLOCK MY FULL RESULTS →"}</button>
              <p style={{ fontSize: 12, color: "rgba(255,255,255,0.4)", textAlign: "center", marginTop: 12, lineHeight: 1.6 }}>
    ...
    return (
      <div style={baseStyle}>
        ...
        <div style={{ paddingTop: 48, paddingBottom: 60 }}>
          ...
          <div style={{ textAlign: "center", marginBottom: 48 }}>
            <div style={{ fontSize: 11, fontWeight: 600, letterSpacing: "0.15em", textTransform: "uppercase", color: "#999", marginBottom: 16 }}>{brandName.toUpperCase()} — YOUR ACTION PLAN</div>
            <div style={{ ...headingStyle, fontSize: 72, color: tier.color, marginBottom: 4 }}>{totalScore}</div>
            <div style={{ fontSize: 14, color: "#999", marginBottom: 12 }}>out of 60</div>
            <div style={{ display: "inline-block", ...headingStyle, fontSize: 16, letterSpacing: "0.15em", padding: "8px 24px", background: tier.color, color: WHITE }}>{tier.label}</div>
          </div>
          ...
          <div style={{ marginBottom: 48 }}>
            <h3 style={{ ...headingStyle, fontSize: 18, marginBottom: 6 }}>YOUR ACTION PLAN</h3>
            <p style={{ fontSize: 14, color: "#888", marginBottom: 16 }}>Here's one thing you can do today for each of your biggest gaps.</p>
              ...
              {weakest.map((cat) => {
                ...
                return (
                  <div key={cat.name} style={{ padding: "20px", background: BLUE_LIGHTEST, borderLeft: `3px solid ${cat.pct < 40 ? "#C0392B" : "#E67E22"}` }}>
                    ...
                    {insight && (
                      <>
                        <div style={{ paddingTop: 12, borderTop: "1px solid rgba(46,75,102,0.1)", marginBottom: 12 }}>
                          <div style={{ fontSize: 12, fontWeight: 600, letterSpacing: "0.08em", textTransform: "uppercase", color: BLUE_DARK, marginBottom: 6 }}>BENCHMARK</div>
                          <p style={{ fontSize: 13, color: "#666", margin: 0, lineHeight: 1.5 }}>{insight.benchmark}</p>
                        </div>
                        <div>
